...
CREATE PROCEDURE `importDebtorCaseFile`(
	in_DBID INT,
...
BEGIN
  DECLARE dbidCount INT;
  DECLARE var_dateNow DATETIME;
  
  SET var_dateNow = CURRENT_TIMESTAMP;
  SET dbidCount = (SELECT Count(dbid) FROM db01_debtor WHERE dbid = in_DBID);
  ...
  IF (dbidCount = 0) THEN
    ...
    SELECT 'insert' as importResult;
  ELSE 
    SELECT 'update' as importResult;
  END IF;
